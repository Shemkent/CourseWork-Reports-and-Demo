...
public class MultiLayerFeedForwardNeuralNetwork extends FeedForwardNeuralNetwork {
	...
	public void backprop(Example example, double alpha) {
		...
		LogisticUnit[] outputUnits = (LogisticUnit[]) getOutputUnits();
		delta[layers-1] = new double[outputUnits.length];
		for(int i=0;i<outputUnits.length;i++){
			LogisticUnit unit = outputUnits[i];

			delta[layers-1][i] = unit.activationPrime(unit.getInputSum())*
					(example.outputs[i]-unit.output);
		}

		//System.out.println(layers-1-1);
		for(int l= layers-1-1;l>=1;l--){//the hidden layers
			int nLayerNodes = this.layers[l].length;
			delta[l] = new double[nLayerNodes];

			//System.out.println(nLayerNodes);

			for(int j = 0; j<nLayerNodes;j++){
				//System.out.println("call");
				double errorShare=0;
				LogisticUnit unit = (LogisticUnit) this.layers[l][j];


				//System.out.println("length:" +unit.outgoingConnections.size());
				int z =0;
				for(Connection i:unit.outgoingConnections){
					//System.out.println(delta[l]);
					errorShare = errorShare+
							(i.weight*delta[l+1][z]);
					//System.out.println("delta = "+delta[l][z]);
					//System.out.println("wei= "+i.weight);

					z++;
				}

				delta[l][j] = unit.activationPrime(unit.getInputSum())
						*errorShare;

				/*System.out.println("prime= " + unit.activationPrime(unit.getInputSum()));
				System.out.println("err= " + errorShare);*/
			}
		}
		...
		for(int l= layers-1;l>=1;l--){
			...
			for(int i=0;i<this.layers[l].length;i++) {
				LogisticUnit unit = (LogisticUnit) this.layers[l][i];

				for(int j=1;j<unit.incomingConnections.size();j++){
					//System.out.println("aaaa " + unit.getWeight(j));
					Connection thisConnection =unit.incomingConnections.get(j);
					double weight = thisConnection.weight +
							(alpha*thisConnection.src.getOutput()*delta[l][i]);

					/*System.out.println(alpha*thisConnection.src.getOutput()*delta[l][i]);
					System.out.println(delta[l][i]);*/

					unit.setWeight(j,weight);
					//System.out.println("bbbb " + unit.getWeight(j));
				}
			}
		}
	}
	...
	public boolean test(Example example) {
		...
		/*for(double i: example.inputs){
			System.out.print(i+" ");
		}
		System.out.println();
		System.out.println(example.outputs[0]+" "+example.outputs[1]+" "+example.outputs[2]);
		for(NeuronUnit i: getOutputUnits()){
